...
import com.makotogroup.sample.wicket.OpenIdRegistrationPage;
...
public class RegistrationService {
    ...
    log.trace("performDiscoveryOnUserSuppliedIdentifier() BEGIN...");
		...
		final List<DiscoveryInformation> discoveries = consumerManager.discover(userSuppliedIdentifier);
    ...
    log.trace("performDiscoveryOnUserSuppliedIdentifier() END...");
		return ret;
	}
	/**
	 ...
	 */
	public static AuthRequest createOpenIdAuthRequest(final DiscoveryInformation discoveryInformation, final String returnToUrl) {
    log.trace("createOpenIdAuthRequest() BEGIN...");
		...
		try {
			...
			fetchRequest.addAttribute("favoriteColor", "http://makotogroup.com/schema/1.0/favoriteColor", false);
			ret.addExtension(fetchRequest);
		} catch (final Exception e) {
			final String message = "Exception occurred while building AuthRequest object!";
			log.error(message, e);
			throw new RuntimeException(message, e);
		}
    log.trace("createOpenIdAuthRequest() END...");
		return ret;
	}
	 ...
	 *  out.
	 */
	public static RegistrationModel processReturn(final DiscoveryInformation discoveryInformation, final PageParameters pageParameters, final String returnToUrl) {
    log.trace("processReturn() BEGIN...");
		...
		try {
			final VerificationResult verificationResult = getConsumerManager().verify(returnToUrl, response, discoveryInformation);
        ...
        ret = new RegistrationModel();
				if (authSuccess.hasExtension(SRegMessage.OPENID_NS_SREG)) {
					final MessageExtension extension = authSuccess.getExtension(SRegMessage.OPENID_NS_SREG);
					if (extension instanceof SRegResponse) {
						ret.setOpenId(verifiedIdentifier.getIdentifier());
						final SRegResponse sRegResponse = (SRegResponse)extension;
						String value = sRegResponse.getAttributeValue("dob");
						if (value != null) {
						  final long millis = Long.valueOf(value).longValue();
	        ...
	        if (extension instanceof FetchResponse) {
	          final FetchResponse fetchResponse = (FetchResponse)extension;
	          ret.setFavoriteColor(fetchResponse.getAttributeValue("favoriteColor"));
	        }
				}
			}
		} catch (final Exception e) {
			final String message = "Exception occurred while verifying response!";
			log.error(message, e);
			throw new RuntimeException(message, e);
		}
    log.trace("processReturn() END...");
		return ret;
	}
  ...
  public static String getReturnToUrl() {
    log.trace("getReturnToUrl() BEGIN...");
    log.trace("getReturnToUrl() END...");
    return RequestUtils.toAbsolutePath(RequestCycle.get().urlFor(OpenIdRegistrationPage.class,
        OpenIdRegistrationPage.createParams(true)).toString());
  }
}
